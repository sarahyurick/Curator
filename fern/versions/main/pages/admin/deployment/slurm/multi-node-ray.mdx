...
If you want many independent pipeline runs over deterministic source shards instead of one shared Ray cluster, use [SLURM Job Arrays](/admin/deployment/slurm-arrays). Each array task can still use `SlurmRayClient` when that individual task spans multiple nodes.
...
- **[SLURM Job Arrays](/admin/deployment/slurm-arrays)** — deterministic source sharding, completion manifests, and retries across independent jobs.
